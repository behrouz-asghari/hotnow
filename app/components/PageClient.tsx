...
export default function PageClient({ data }: { data: AnalyzeResponse }) {
  ...
  const generalReport = data.reports.generalReport;
  const sentiment = generalReport?.sentiment ?? {
    fear: 0,
    excitement: 0,
    crisis: 0,
    sexualSignal: 0,
        ...
        <div className="flex flex-row justify-between items-center gap-4 w-full">
          <div className="flex items-center gap-1">
            ...
            <div className="flex flex-col items-start">
              <h1 className="text-1xl font-extrabold">داشبورد AI ترندها</h1>
              <span className="text-xs text-slate-400 block mt-1">
                تحلیل آنچه دیتای آزاد است
              </span>
            </div>
          </div>
